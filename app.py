...
# Detect CPU Cores & set dynamic slider bounds (Robust for 1-core cloud instances)
detected_cpus = max(1, multiprocessing.cpu_count())
max_slider_val = max(8, detected_cpus)
...
# Section 2: Worker Configuration (Fixed for 1-core cloud VMs)
...
worker_count = st.sidebar.slider(
    ...
    step=1,
    help=f"Detected CPU cores on server: {detected_cpus}."
)
